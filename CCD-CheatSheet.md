...
|jd GUI|https://java-decompiler.github.io/|Good to view class files. To obtain class files, for example, extract the class files from `log4j-core-<version number>.jar` by copying out to other directory (due to write protection) then rename it to change extension to `.zip`. Unzip the file and it will provide a load of class files. Upload the folder to jd GUI and view the class files code as notepad can't view the code properly|
